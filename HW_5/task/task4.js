...
function stringDot(word) {
    var vow = ['a', 'o', 'y', 'e', 'u', 'i'];
    var loverWord = word.toLowerCase();
    var splitted = loverWord.split('');

    splitted.forEach((char, charId, splitted) => {
        if(vow.indexOf(char) > -1){
        splitted.splice(charId, 1, '');
    }
});
    var lineWithChanges = splitted
        .filter(symbol => symbol !== '')
        .map(symbol => `.${symbol}`)
        .join('');

    return lineWithChanges;
}
